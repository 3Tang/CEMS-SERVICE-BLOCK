...
public class DeviceStateChangeProcess {
	...
	public static synchronized void changeProcess(List<ProtectedDevice> protectedDevs) {
		...
		{	
			...
			log.info("BLOCK_TARGET_LIST缓存已经建立!已经存储的设备数为:"+blocktargetMap.size());
		}
		...
		if (protectedDevs != null && protectedDevs.size() > 0) {
			...
			log.info("添加的BLOCK_TARGET_LIST的数目为:"+blocktargetMap.size());
		...
		if (onlineOutData!= null) {
			...
			String hostNum=String.valueOf(attckDevList.size());
			//把在线返回的数据存入阻断主机列表
			for (AttackDeviceInfoBean attackBean : attckDevList) {
				//需要把在线传过来的数据倒装进阻断主机列表
				...
				AttackDeviceInfoBean desBean = JsonUtils.jsonToBlockBean(
						...
						BlockHostBean.class);
				BlockInfo blockIF=new BlockInfo();
				BlockDevice blockDevice=new BlockDevice();
				List<BlockInfo> blockIFList=new ArrayList<BlockInfo>();
				List<BlockHostBean> blockdeviceList=desBean.getBlockHost();
				String sendTime=DateUtils.format(new Date());
				...
				for(BlockHostBean blockhost:blockdeviceList)
				{
					if(StringUtils.isNotBlank(blockhost.getDevOnlyId())){
						...
						if(StringUtils.isNotBlank(attackBean.getIp()))
						{
							...
							blockDevice.setBlockIp(blockhost.getIp());
							blockDevice.setUdpPort(blockhost.getUdpPort());
							blockDevice.setRouteIp(blockhost.getRouteIp());
							blockDevice.setSendTime(sendTime);
							blockIF.setIp(desBean.getIp());
							blockIF.setMac(desBean.getMac());
							blockIF.setBlockHostNum(hostNum);
							if(blockDevMap.containsKey(innerKey))
							{
								List<BlockInfo> oldBL=blockDevMap.get(innerKey).getBlockInfo();
								oldBL.add(blockIF);
								blockDevice.setBlockInfo(oldBL);
							}
							else
							{
								blockIFList.add(blockIF);
								blockDevice.setBlockInfo(blockIFList);
							}
			...
			log.info("从在线服务获取的BLOCK_HOST_LIST的数目为:"+blockDevMap.size());
	...
	private static ByteBuffer getOnlineNo7IfData(String blockHostNum) {
		...
		log.info("向在线服务7号接口发送的被阻断设备数为:"+deviceBeans.size());
		...
		IOnlineService client = ServiceFactory.getService(IOnlineService.class, 10000);
