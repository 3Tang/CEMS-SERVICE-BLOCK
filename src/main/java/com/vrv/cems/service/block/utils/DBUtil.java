...
public class DBUtil {
	...
	public static DBServParamBean queryByServiceCode(String serviceCode) {
		...
		DBServParamBean serviceParam = new DBServParamBean();
		String sql = "select * from " + SERVICEPARAM_TABLE_NAME
	...
	public static ProtectedDevice getProtectState(ScanProcessBean deviceIn) {
		    ProtectedDevice protectedBean = new ProtectedDevice();
			MapHandler mapHadler = new MapHandler();
			String ip = deviceIn.getIp();
			String mac = deviceIn.getMac();
			String protectState = "";
			String querySql = "select ip,mac,protectState,isOpened,isFireWall from cems_device_unreg where ip=? and mac=?" ;
			Object[] queryParams = { ip,mac };
			...
			if (map != null && map.size() > 0) {
				...
				protectedBean.setProtectState(protectState);
			}
		
			protectedBean.setIp(ip);
			protectedBean.setMac(mac);
	...
	public static boolean deviceBlockContais(DeviceBean bean) {
		...
		BeanHandler<DeviceBlockBean> beanHandler = new BeanHandler<DeviceBlockBean>(DeviceBlockBean.class);
		try {
	...
	* @throws 
	*/
	public static boolean isRoam(String devOnlyId) {
		MapHandler mapHadler = new MapHandler();
		String querySql = "select devOnlyId,roamState from cems_device where devOnlyId=?" ;
		Object[] queryParams = {devOnlyId};
		String roamState="";
		Map<String, Object> map = new HashMap<String, Object>();
		try {
		...
		if (map != null & map.size() > 0) {
			try {
				roamState = map.get("roamState").toString();
			} catch (NullPointerException e) {
				roamState = "0";
			}
		}
		
		if(roamState.equals("1"))
		{
			return true;
		}
		//非漫游
